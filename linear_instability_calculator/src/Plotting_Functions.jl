...
#          ylim = (m[1], m[end])
    ...
    for iEig in collect(1:Neigs)
        ## find max ω for some k and m
        ω_max = 0
        best_ik = 0
        best_im = 0
        for (ik, k) in enumerate(ks)
            tmp              = findmax(imag(ωs/fz)[iEig, :, ik])
            max_growth, im   = tmp[1], tmp[2]
        ...
        ## reassign variables to match (need to change this to be less confusing) TODO
        ik = best_ik
        indm = best_im
        max_growth = ω_max
        ...
        u_plt = contour(Y, Z, U, title="u"; kwargs...)
        v_plt = contour(Y, Z, V, title="v"; kwargs...)
        b_plt = contour(Y, Z, B, title="b"; kwargs...)
